...
FishFish is an automated service designed to quickly detect and mitigate phishing threats before they can be exploited for malicious purposes.

Learn more about Anti-Phishing Detection at [fishfish.gg](https://fishfish.gg/).
...
$detections = $client->Detection()->list();
print_r(array_map(fn($item) => $item->data_get(), $detections));
```
