...
// AntiPhishingDetection SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = AntiPhishingDetectionOmni::makeStructRunner(
    ...
    {
        return AntiPhishingDetectionOmni::objmodel(AntiPhishingDetectionOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return AntiPhishingDetectionOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return AntiPhishingDetectionOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(AntiPhishingDetectionOmni::structfix($now, Struct::undef()))
                        !== json_encode(AntiPhishingDetectionOmni::structfix($val, Struct::undef()))) {
