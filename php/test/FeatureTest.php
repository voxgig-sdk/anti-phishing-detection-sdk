...
// AntiPhishingDetection SDK feature test
//
...
class FtCtrl extends AntiPhishingDetectionControl
{
...
class FtClient
{
    ...
    private AntiPhishingDetectionUtility $utility;

    public function __construct(AntiPhishingDetectionUtility $utility, array $options)
    {
    ...
    public function get_utility(): AntiPhishingDetectionUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (AntiPhishingDetectionContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public AntiPhishingDetectionContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (AntiPhishingDetectionContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = AntiPhishingDetectionConfig::make_feature($name);
    ...
    private static function buildUrl(AntiPhishingDetectionSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof AntiPhishingDetectionError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new AntiPhishingDetectionResponse($fetched);
        }
    ...
    private function fail(AntiPhishingDetectionContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (AntiPhishingDetectionContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
