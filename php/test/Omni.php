...
//    only - a typed AntiPhishingDetectionContext there would make every ctx
...
final class AntiPhishingDetectionOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new AntiPhishingDetectionContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new AntiPhishingDetectionSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new AntiPhishingDetectionResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new AntiPhishingDetectionError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new AntiPhishingDetectionResponse($clean['response']);
    ...
    public static function err_from_map($m): ?AntiPhishingDetectionError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new AntiPhishingDetectionError(is_string($code) ? $code : '', $msg);
    }
