...
The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->Detection()` — with named operations (`list`/`create`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
print_r(array_map(fn($item) => $item->data_get(), $detection));
```
...
```php
$client = AntiPhishingDetectionSDK::test($testopts, $sdkopts);
```
...
```
php/
├── antiphishingdetection_sdk.php          -- Main SDK class
...
The main class (`antiphishingdetection_sdk.php`) exports the SDK class
...
// $detection->data_get() now returns the detection data from the last list
