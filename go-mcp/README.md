# anti-phishing-detection-mcp

[MCP](https://modelcontextprotocol.io) server exposing the AntiPhishingDetection SDK as
two agent tools — `anti-phishing-detection_list` and `anti-phishing-detection_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/anti-phishing-detection-mcp)
make build

# 2. Provide credentials via the environment
export ANTI_PHISHING_DETECTION_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user anti-phishing-detection \
  -- /absolute/path/to/anti-phishing-detection-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./anti-phishing-detection-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// anti-phishing-detection_list: first page of records
{ "entity": "detection" }
{ "entity": "detection", "query": { } }

// anti-phishing-detection_load: one record by id
{ "entity": "detection", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/anti-phishing-detection-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user anti-phishing-detection \
     -- "$PWD"/dist/*/anti-phishing-detection-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `anti-phishing-detection_list` and `anti-phishing-detection_load` tools now appear
   in new sessions. Ask the agent to *"list detection using anti-phishing-detection"*
   and it calls `anti-phishing-detection_list` with `{"entity":"detection"}`.
...
```sh
export ANTI_PHISHING_DETECTION_APIKEY=sk_live_xxx            # API key
export ANTI_PHISHING_DETECTION_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `anti-phishing-detection_list` tool
...
### Call the `anti-phishing-detection_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `anti-phishing-detection_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `anti-phishing-detection_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `ANTI_PHISHING_DETECTION_APIKEY` | API key sent with every request. |
| `ANTI_PHISHING_DETECTION_BASE` | Optional override of the API base URL. |
...
```sh
./anti-phishing-detection-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"anti-phishing-detection_load","arguments":{"entity":"detection","query":{"id":1}}}}'
```
