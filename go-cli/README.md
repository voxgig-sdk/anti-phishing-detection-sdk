...
boru-driven command-line client **and** interactive REPL for the AntiPhishingDetection
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/anti-phishing-detection-cli)
make build

# 2. See usage (words, entities, env vars)
./anti-phishing-detection-cli --help
...
ANTI_PHISHING_DETECTION_BASE=https://api.example.com ./anti-phishing-detection-cli list detection

# 6. No arguments -> interactive REPL
./anti-phishing-detection-cli
anti-phishing-detection> list detection
anti-phishing-detection> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/anti-phishing-detection-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/anti-phishing-detection-cli list detection
   ```
...
Run with no arguments to open a REPL (prompt `anti-phishing-detection>`). Each line is
evaluated as its own boru expression:

```text
$ ./anti-phishing-detection-cli
anti-phishing-detection> list detection
anti-phishing-detection> /help
anti-phishing-detection> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `detection`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/anti-phishing-detection-cli`. |
...
(`./anti-phishing-detection-cli <expr>`) and interactively (the REPL), and expressions compose the
