...
class TestDetectionEntity:
    ...
    def test_should_stream(self):
        ...
        base = AntiPhishingDetectionSDK.test(seed, None)
        seen = list(base.Detection(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = AntiPhishingDetectionSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Detection(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _detection_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "detection." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set ANTI_PHISHING_DETECTION_TEST_DETECTION_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        detection_ref01_ent = client.Detection(None)
        detection_ref01_data = helpers.to_map(vs.getprop(
            vs.getpath(setup["data"], "new.detection"), "detection_ref01"))

        detection_ref01_data = helpers.to_map(runner.entity_data(detection_ref01_ent.create(detection_ref01_data, None)))
...
def _detection_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/detection/DetectionTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("ANTI_PHISHING_DETECTION_TEST_DETECTION_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("ANTI_PHISHING_DETECTION_TEST_LIVE") == "TRUE":
        ...
        client = AntiPhishingDetectionSDK(helpers.to_map(merged_opts))

    _live = env.get("ANTI_PHISHING_DETECTION_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("ANTI_PHISHING_DETECTION_TEST_EXPLAIN") == "TRUE",
        "live": _live,
