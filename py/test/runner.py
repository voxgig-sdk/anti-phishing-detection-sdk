# AntiPhishingDetection SDK test runner
...
class AntiPhishingDetectionTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            AntiPhishingDetectionTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = AntiPhishingDetectionTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = AntiPhishingDetectionTestRunner.getenv("ANTI_PHISHING_DETECTION_TEST_LIVE")
        override = AntiPhishingDetectionTestRunner.getenv("ANTI_PHISHING_DETECTION_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = AntiPhishingDetectionTestRunner.getenv(key)
        ...
        explain = AntiPhishingDetectionTestRunner.getenv("ANTI_PHISHING_DETECTION_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if AntiPhishingDetectionTestRunner._test_control is not None:
            return AntiPhishingDetectionTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                AntiPhishingDetectionTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            AntiPhishingDetectionTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = AntiPhishingDetectionTestRunner.load_test_control()
...
def load_env_local():
    AntiPhishingDetectionTestRunner.load_env_local()


def env_override(m):
    return AntiPhishingDetectionTestRunner.env_override(m)


def entity_data(v):
    return AntiPhishingDetectionTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return AntiPhishingDetectionTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return AntiPhishingDetectionTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return AntiPhishingDetectionTestRunner.load_test_control()


def live_client_options():
    return AntiPhishingDetectionTestRunner.live_client_options()


def live_delay_ms():
    return AntiPhishingDetectionTestRunner.live_delay_ms()
