...
from antiphishingdetection_sdk.antiphishingdetection_types import (
    Detection,
    DetectionListMatch,
    DetectionCreateData,
)
...
class DetectionEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DetectionEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Detection]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Detection().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
