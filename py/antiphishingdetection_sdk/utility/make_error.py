...
def make_error_util(ctx, err):
    if ctx is None:
        from antiphishingdetection_sdk.core.context import AntiPhishingDetectionContext
        ctx = AntiPhishingDetectionContext({}, None)
    ...
    result = ctx.result
    if result is None:
        result = AntiPhishingDetectionResult({})
    result.ok = False
    ...
    errmsg = ""
    if isinstance(err, AntiPhishingDetectionError):
        errmsg = err.msg
    ...
    sdk_err = AntiPhishingDetectionError("", msg, ctx)
