...
from antiphishingdetection_sdk.core.helpers import get_ctx_prop, to_map
...
class AntiPhishingDetectionContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = AntiPhishingDetectionControl()
        ...
        if isinstance(sp, AntiPhishingDetectionSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, AntiPhishingDetectionResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, AntiPhishingDetectionResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return AntiPhishingDetectionOperation({})
        ...
        op = AntiPhishingDetectionOperation({
            "entity": entname,
            "name": opname,
            "input": inpt,
            "points": points,
    ...
    def make_error(self, code, msg):
        return AntiPhishingDetectionError(code, msg, self)
