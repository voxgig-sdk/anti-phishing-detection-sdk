...
module AntiPhishingDetectionOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if AntiPhishingDetectionOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = AntiPhishingDetectionOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| AntiPhishingDetectionOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || AntiPhishingDetectionOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = AntiPhishingDetectionSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = AntiPhishingDetectionError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { AntiPhishingDetectionOmni.errify(err) }
