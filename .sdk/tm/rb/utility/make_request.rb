...
module AntiPhishingDetectionUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = AntiPhishingDetectionResponse.new({})
    ...
    if fetch_err
      ...
      response = AntiPhishingDetectionResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = AntiPhishingDetectionResponse.new(fetched)
    else
