...
// AntiPhishingDetection SDK retry feature
...
class AntiPhishingDetectionRetryFeature extends AntiPhishingDetectionBaseFeature
{
    ...
    public function init(AntiPhishingDetectionContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (AntiPhishingDetectionContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(AntiPhishingDetectionContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(AntiPhishingDetectionContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
