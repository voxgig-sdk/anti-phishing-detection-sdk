...
describe("DetectionEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Detection(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Detection(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Detection(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = detection_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "list"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "detection." .. _op, _live and "live" or "unit")
    ...
    -- CREATE
    local detection_ref01_ent = client:Detection(nil)
    local detection_ref01_data = helpers.to_map(vs.getprop(
...
function detection_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/detection/DetectionTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read detection test data: " .. entity_data_file)
  end
