...
local DetectionEntity = {}
DetectionEntity.__index = DetectionEntity


function DetectionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DetectionEntity)
  self._name = "detection"
  self._client = client
...
function DetectionEntity:get_name()
  return self._name
end


function DetectionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DetectionEntity.new(self._client, opts)
end
...
function DetectionEntity:mark_deleted()
  self._deleted = true
end


function DetectionEntity:deleted()
  return true == self._deleted
end


function DetectionEntity:data_set(args)
  if args ~= nil then
...
function DetectionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DetectionEntity:match_set(args)
  if args ~= nil then
...
function DetectionEntity:match_get()
...
function DetectionEntity:stream(action, args, callopts)
...
---@param reqmatch DetectionListMatch
---@param ctrl? table
---@return Detection[]
---@return string? err
function DetectionEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata DetectionCreateData
---@param ctrl? table
---@return Detection
---@return string? err
function DetectionEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function DetectionEntity:_run_op(ctx, post_done)
  local utility = self._utility
