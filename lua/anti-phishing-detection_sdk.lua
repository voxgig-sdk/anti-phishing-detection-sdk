...
require("anti-phishing-detection_types")
...
local AntiPhishingDetectionSDK = {}
AntiPhishingDetectionSDK.__index = AntiPhishingDetectionSDK
...
AntiPhishingDetectionSDK._make_feature = _make_feature


function AntiPhishingDetectionSDK.new(options)
  local self = setmetatable({}, AntiPhishingDetectionSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function AntiPhishingDetectionSDK:options_map()
...
function AntiPhishingDetectionSDK:get_utility()
  return Utility.copy(self._utility)
end


function AntiPhishingDetectionSDK:get_root_ctx()
  return self._rootctx
end


function AntiPhishingDetectionSDK:prepare(fetchargs)
...
function AntiPhishingDetectionSDK:direct(fetchargs)
...
function AntiPhishingDetectionSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function AntiPhishingDetectionSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "AntiPhishingDetectionSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function AntiPhishingDetectionSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function AntiPhishingDetectionSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "AntiPhishingDetectionSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function AntiPhishingDetectionSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = AntiPhishingDetectionSDK.new(sdkopts)
  sdk.mode = "test"
...
return AntiPhishingDetectionSDK
