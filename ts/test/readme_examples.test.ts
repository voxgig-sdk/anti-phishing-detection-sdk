...
//     (\`new AntiPhishingDetectionSDK(...)\` / \`AntiPhishingDetectionSDK.test(...)\` become
//     \`AntiPhishingDetectionSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'AntiPhishingDetectionSDK'
...
const TEST_SEED = {"entity":{"detection":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new AntiPhishingDetectionSDK(...)\` and any
// \`AntiPhishingDetectionSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+AntiPhishingDetectionSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/AntiPhishingDetectionSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
