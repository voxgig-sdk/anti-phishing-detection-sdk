...
import { AntiPhishingDetectionSDK } from '../../..'
...
describe('DetectionDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new AntiPhishingDetectionSDK({
  ...
  test('direct-list-detection', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-detection', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['ANTI_PHISHING_DETECTION_TEST_DETECTION_ENTID']
