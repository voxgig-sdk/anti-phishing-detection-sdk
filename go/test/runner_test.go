...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("ANTIPHISHINGDETECTION_TEST_LIVE") == "TRUE" ||
		os.Getenv("ANTIPHISHINGDETECTION_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("ANTIPHISHINGDETECTION_TEST_EXPLAIN"); explain != "" {
