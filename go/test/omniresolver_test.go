...
import (
	...
	omni "github.com/voxgig-sdk/anti-phishing-detection-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.AntiPhishingDetectionError); is {
				out := map[string]any{
					"name":    "AntiPhishingDetectionError",
...
func omniCtx(arg any, client *sdk.AntiPhishingDetectionSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.AntiPhishingDetectionSDK); is {
			client = live
