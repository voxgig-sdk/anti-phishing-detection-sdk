...
class DetectionEntity
  ...
  def make
    opts = @_entopts.dup
    DetectionEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = AntiPhishingDetectionHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = AntiPhishingDetectionHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [DetectionListMatch, Hash, nil] match filter (any subset of
  #   Detection fields); defaults to nil, treated as an empty match that lists all.
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = AntiPhishingDetectionHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
