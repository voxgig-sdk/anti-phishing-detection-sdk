...
class DetectionDirectTest < Minitest::Test
  def test_direct_list_detection
    setup = detection_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-detection", setup[:live] ? "live" : "unit")
    if _should_skip
...
def detection_direct_setup(mockres)
  ...
  if live
    ...
    client = AntiPhishingDetectionSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
